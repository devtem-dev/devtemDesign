...
window.devtemDesign = function(appid="app-root", config) {
  ...
  // ---- NAVIGATION ----
  const navLinksItems = (config.navLinks || ['Discover', 'Studio', 'Archive', 'Journal'])
    .map(link => {
  ...
  const brandChips = (config.brands || []).map((b, idx) =>
    `<span style="font-family:monospace;font-weight:500;color:${idx%2===0?'var(--accent)':'var(--accent2)'};">${b.toUpperCase()}</span>`
  ...
  heroSection.innerHTML = `
    ...
    ${config.brands && config.brands.length ? `
      <div class="hero-inline-brands">
        <div style="display:flex;flex-wrap:wrap;justify-content:center;gap:24px;align-items:center;">${brandChips}</div>
        <div style="height:1px;background:linear-gradient(90deg,transparent,var(--accent),transparent);margin:22px 0 12px;"></div>
        <div style="color:var(--muted);font-size:0.75rem;text-align:center;">${config.brandsSubtitle || 'global innovators, design pioneers'}</div>
      </div>` : ''}
  `;
  ...
  // ---- FOOTER (fully declarable) ----
  /*
   * config.footer shape (all optional — falls back gracefully):
   * {
   *   tagline: "string",          // short brand tagline below logo
   *   columns: [                  // link columns
   *     { heading: "Product", links: [{ label: "Docs", href: "/docs" }, ...] },
   *     { heading: "Company", links: [...] },
   *   ],
   *   social: [                   // social icon links (Font Awesome class + href)
   ...
   *   ],
   *   copy: "© 2026 Figsh Digital. All rights reserved."  // override copyright line
   * }
   *
   * Legacy support: config.footerCopy still works as the copyright string
   * when config.footer is absent or config.footer.copy is absent.
   */
  const footerCfg    = config.footer || {};
  const copyText     = footerCfg.copy || config.footerCopy || `© ${new Date().getFullYear()} Design engine`;
  const tagline      = footerCfg.tagline || '';
  const columns      = footerCfg.columns || [];
  const social       = footerCfg.social  || [];
  const legalLinks   = footerCfg.legal   || [];

  // Build link columns HTML
  const columnsHtml = columns.map(col => `
    <div class="footer-col">
      <div class="footer-col-heading">${col.heading}</div>
      <ul class="footer-col-links">
        ${(col.links || []).map(l => `<li><a href="${l.href || '#'}" class="footer-link">${l.label}</a></li>`).join('')}
      </ul>
    </div>
  `).join('');
  ...
  // Legal links HTML
  const legalHtml = legalLinks.map(l => `<a href="${l.href || '#'}" class="footer-legal-link">${l.label}</a>`).join('');
    ...
    <div class="footer-body">
      <div class="footer-brand-col">
        <span class="footer-logo">
          <img src="${config.logo || 'https://fscss.devtem.org/assets/images/fscss.png'}" width="26" height="26">
          ${(config.logoName || 'CORE').toUpperCase()}
        </span>
        ${tagline ? `<p class="footer-tagline">${tagline}</p>` : ''}
        ${socialHtml ? `<div class="footer-socials">${socialHtml}</div>` : ''}
      </div>
      ${columnsHtml ? `<div class="footer-cols">${columnsHtml}</div>` : ''}
    </div>
    <div class="footer-bottom">
      <span class="footer-copy">${copyText}</span>
  ...
  function initInteractions() {
    ...
    if (window.matchMedia('(pointer: fine)').matches) {
      ...
      document.addEventListener('click', (e) => {
        ...
        rippleEl.style.cssText = `position:absolute;width:12px;height:12px;border-radius:50%;border:1px solid var(--accent);left:${e.clientX}px;top:${e.clientY}px;transform:translate(-50%,-50%) scale(0);animation:rippleAnim 0.7s ease-out forwards;`;
